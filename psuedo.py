...
inputs = set()
stateVars = {'CR0' : '64', 'CS': '64', 'SS': '64', 'cs_selector': '16', 'cs_base': '32', 'cs_limit': '20', 'cs_accessRights': '12', 'cpl': '2', 'ss_selector': '16', 'ss_base': '32', 'ss_limit': '20', 'ss_accessRights': '12', 'EFER': '64', 'CR4': '64', 'rflags': '64', 'rcx': '64', 'rip': '64'}
...
ss_accessRightsDict = {'TYPE': 'State.ss_accessRights # [11:8]', 'S': 'State.ss_accessRights # [7:7]', 'DPL': 'State.ss_accessRights # [6:5]', 'P': 'State.ss_accessRights # [4:4]', 'AVL': 'State.ss_accessRights # [3:3]', 'L': 'State.ss_accessRights # [2:2]', 'D': 'State.ss_accessRights # [1:1]', 'B': 'State.ss_accessRights # [1:1]', 'G''D': 'State.ss_accessRights # [0:0]'}
    ...
    def convertIntel(self, filename):
        ...
        for line in readFile:
            ...
            if line.lstrip().startswith("IF") and " = " not in line:
                ...
                numSpaces += 4
            elif line.lstrip().startswith("THEN") or line.lstrip().startswith("END"):
                pass
            elif line.lstrip().startswith("ELSE"):
                if "#" in line:
                    newLine = line.split(" #")
                    writeFile.write(" "*(numSpaces-4) + newLine[0].strip().replace("ELSE", "else") + ": #" + newLine[1])
                else:
                    writeFile.write(" "*(numSpaces-4) + line.strip().replace("ELSE", "else") + ":\n")
...
class astVisit(ast.NodeVisitor):
    ...
    def visit_Attribute(self, node):
        return str(self.visit(node.value) + "." + node.attr)
    ...
    def visit_Assign(self, node, condition = None):
        assign = str(self.visit(node.targets[0])),
        assign += "=",
        assign = " ".join(assign)
        assign += " " + str(self.visit(node.value))
        if assign in self.varDict:
            if self.varDict[assign] is list:
                self.varDict[assign].append(condition)
            else:
                self.varDict[assign] = [self.varDict[assign], condition]
        else:
            self.varDict[assign] = condition
        return 
...
class modulePrint():
    ...
    def __init__(self, filename, varDict):
        self.filename = filename
        self.inputs = set()
        self.constants = set()
        self.defines = set()
        ...
        #[[var, next[var], case]]
        for key in varDict:
            self.cvd.append([key.split("=")[0].strip(), key.split("=")[1].strip(), varDict[key]])
        ...
        for line in file:
            ...
            for word in line.split():
                ...
                if word in AMDDict:
                    ...
                    self.defines.add(word + " := " + EFERDict[word.replace("EFER", "")])
                    ...
                    self.defines.add(word + " := " + cs_accessRightsDict[word])
                    ...
                    self.defines.add(word + " := " + ss_accessRightsDict[word])
    ...
    def printVar(self):
        ...
        for var in self.cvd:
            if var[0] == 'exitStatus':
                continue
            elif var[0].startswith("RFLAGS") or var[0] in rflagsDict:
                if rflags:
                    rflags = False
                    print "rflags : [64];"
            else:
                print str(var[0]) + " : [" + str(stateVars[var[0]]) + "];"
    ...
    def printAssign(self):
        ...
        for var in self.cvd:
            if var[0] == "exitStatus":
                ...
                init = 0
                default = "State." + var[0]
            print "init[" + var[0] + "] := "+ default +";"
            print "next[" + var[0] + "] := case"
            if type(var[2]) is list:
                for case in var[2]:
                    print tab + str(case).replace("=1", "=b1").replace("=3", "=b1@b1") + " : " + var[1].replace("1", "=b1") + ";"
            else:
                print tab + str(var[2]).replace("=1", "=b1").replace("=3", "=b1@b1") + " : " + var[1].replace("1", "b1") + ";"
            print tab + "default : " + default + ";"
            print "esac;"
            print

    def write(self):
        print "MODULE "+ self.filename.replace("Python.txt", "")
        self.findInputs()
        self.printInput()
        self.printVar()
        self.printConsts()
...
if __name__ == "__main__":
    ...
    writer = modulePrint(varDict = v.varDict, filename = filename)
    writer.write()
    if not args.keep:
